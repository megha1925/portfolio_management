...
import com.cts.calculatenetworth.exception.UserNotFoundException;
...
public class NetworthController {
	static  final String UNAUTHORISED = "UNAUTHORIZED_REQUEST";
	...
	public PortfolioDetails getAssetById(@RequestHeader("Authorization") String authorization) throws UserNotFoundException {
		if(!authorizationClient.authorizeRequest(authorization)){
			throw new UserNotFoundException(UNAUTHORISED);
		}
		else{
	...
	public double calculateNetworth(@RequestHeader("Authorization") String authorization) throws UserNotFoundException {
		if(!authorizationClient.authorizeRequest(authorization)){
			throw new UserNotFoundException(UNAUTHORISED);
		}
			int pid = authorizationClient.getUserId(authorization);
	...
	public ResponseEntity<AssetSaleResponse> sellingAssets(@RequestHeader("Authorization") String authorization,
			@RequestBody List<SaleDetailsVO> saleDetailsList) throws UserNotFoundException {
		if(!authorizationClient.authorizeRequest(authorization)){
			throw new UserNotFoundException(UNAUTHORISED);
		}
			int pid = authorizationClient.getUserId(authorization);
